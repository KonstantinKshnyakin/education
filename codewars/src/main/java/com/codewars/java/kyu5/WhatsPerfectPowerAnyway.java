 ...
 * <p>
 * Note: For a perfect power, there might be several pairs. For example 81 = 3^4 = 9^2, so (3,4) and (9,2)
 * are valid solutions. However, the tests take care of this, so if a number is a perfect power, return
...
public class WhatsPerfectPowerAnyway {
    ...
    public static int[] isPerfectPower(int n) {
        ...
        for (int i = 2; i < Integer.MAX_VALUE; i++) {
            int j = (int) Math.round(Math.pow(n, 1.0 / i));
            if (j <= 1) {
                break;
            } else if (Math.pow(j, i) == n) {
                return new int[]{j, i};
            }
        }
        return null;
    ...
    public void testUpTo500() {
        ...
        for (int i : pp) assertNotNull(isPerfectPower(i), i + " is a perfect power");
    }
