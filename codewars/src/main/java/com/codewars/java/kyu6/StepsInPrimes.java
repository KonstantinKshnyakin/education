 ...
 * if these g-steps prime numbers exist otherwise nil or null or None or Nothing or [] or "0, 0" or {0, 0} or 0 0
 ...
 *
 * Notes:
 * The idea of "step" is close to that of "gap" but it is not exactly the same. For those interested they can have
...
public class StepsInPrimes {
    ...
    public static long[] step(int g, long m, long n) {
        BigInteger previous = BigInteger.valueOf(m - 1).nextProbablePrime();
        BigInteger limit = BigInteger.valueOf(n);
        BigInteger step = BigInteger.valueOf(g);
        List<BigInteger> prevPrimes = new ArrayList<>();
        for (BigInteger next = previous.nextProbablePrime(); next.compareTo(limit) <= 0; previous = next, next = next.nextProbablePrime()) {
            prevPrimes.add(previous);
            for (BigInteger prevPrime : prevPrimes) {
                if (next.subtract(prevPrime).equals(step)) {
                    return new long[]{prevPrime.longValue(), next.longValue()};
                }
            }
        }
        return null;
